...
class ToggleEnabled extends \W5n\Model\Field\Validation\Validation
{
    protected $fields         = [];
    ...
    public function beforeRenderField(\W5n\Html\Tag $html, \W5n\Model\Field\Field $field)
    {
        ...
        if (!empty($toShow)) {
            $toShowSelectorOn   = '$("' . implode(', ', $toShow) . '").removeAttr("disabled");';
            $toShowSelectorOff  = '$("' . implode(', ', $toShow) . '").attr("disabled", true);';
        }
        
        if (!empty($toHide)) {
            $toHideSelectorOn  = '$("' . implode(', ', $toHide) . '").removeAttr("disabled")';
            $toHideSelectorOff = '$("' . implode(', ', $toHide) . '").attr("disabled", true);';
        }
